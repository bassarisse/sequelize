...
var QueryGenerator = {
  ...
  createTableQuery: function(tableName, attributes, options) {
    ...
    for (var attr in attributes) {
      if (attributes.hasOwnProperty(attr)) {
        var dataType = Utils._.template(attributes[attr])({ tableName: tableName })
          , handledDataType = null;

        if (Utils._.includes(dataType, 'REFERENCES')) {
           // MSSQL doesn't support inline REFERENCES declarations: move to the end

          var match = dataType.match(/^(.+) (REFERENCES.*)$/)
            , constraintMatch = dataType.match(/(CONSTRAINT +[^ ]+ )/g)
            , foreignKeyConstraint = "";

          handledDataType = match[1];

          if (Array.isArray(constraintMatch)) {
            if (constraintMatch.length === 2) {
              foreignKeyConstraint = constraintMatch[1];
            } else if (constraintMatch.length === 1) {
              if (dataType.split(constraintMatch[0])[1].indexOf('DEFAULT') === -1) {
                foreignKeyConstraint = constraintMatch[0];
              }
            }
          }

          if (foreignKeyConstraint) {
            handledDataType = handledDataType.replace(foreignKeyConstraint.trim(), ' ');
          } else {
            foreignKeyConstraint = 'CONSTRAINT ' + this.quoteIdentifier('FK_' + tableName + '_' + attr) + ' ';
          }
          ...
          handledDataType = dataType;
        }

        if (Utils._.includes(dataType, 'PRIMARY KEY')) {
          primaryKeys.push(attr);
          handledDataType = handledDataType.replace(/PRIMARY KEY/, '');
        }

        if (handledDataType) {
          attrStr.push(this.quoteIdentifier(attr) + ' ' + handledDataType);
        }
    ...
    , pkString = primaryKeys.map(function(pk) { return self.quoteIdentifier(pk); }).join(', ');
    ...
    if (pkString.length > 0) {
      values.attributes += ', CONSTRAINT ';
      if (Utils._.isString(options.primaryKeyConstraint)) {
        values.attributes += this.quoteIdentifier(options.primaryKeyConstraint);
      } else {
        values.attributes += this.quoteIdentifier('PK_' + tableName);
      }
      values.attributes += ' PRIMARY KEY (' + pkString + ')';
    }

    values.attributes += foreignKeys.map(function(fk) {
      return ', ' + (fk.constraint || '') + ' FOREIGN KEY (' + self.quoteIdentifier(fk.attr) + ') ' + fk.sql;
    }).join('');
  ...
  addColumnQuery: function(table, key, dataType) {
    ...
    return Utils._.template(query)({
      ...
      attribute: Utils._.template(attribute)({
        tableName: table
      })
    });
  },
  ...
  attributeToSQL: function(attribute) {
    ...
    if (attribute.type !== 'TEXT' && attribute.type._binary !== true &&
      ...
      if (attribute.defaultValueConstraint) {
        template += ' CONSTRAINT ' + this.quoteIdentifier(attribute.defaultValueConstraint);
      } else {
        var tableName = attribute.Model && attribute.Model.tableName ? attribute.Model.tableName : '<%= tableName %>';
        template += ' CONSTRAINT ' + this.quoteIdentifier('DF_' + tableName + '_' + attribute.field);
      }
      template += ' DEFAULT ' + this.escape(attribute.defaultValue);
    }

    if (attribute.constraint) {
      template += ' CONSTRAINT ' + this.quoteIdentifier(attribute.constraint);
    }
